...
def plotDensities(effectiveRanksDF):
    ...
    x, bins, p = axes[0][0].hist(nbVertices, bins=np.logspace(np.log10(0.1),
                                                              np.log10(21000),
                                                              40),
    ...
    axes[0][0].set_ylim([-0.02*0.20, 0.20])
    axes[0][0].set_xlim([0.5, 9*10**4])
    axes[0][0].set_xticks([10**0, 10**2, 10**4])

    rankN = effectiveRanksDF['Rank']/effectiveRanksDF['Size']
    x, bins, p = axes[0][1].hist(rankN, bins=30, range=(0, 1),
                                 density=True, color=color)
    for item in p:
        item.set_height(item.get_height()/sum(x))
    ...
    x, bins, p = axes[0, 2].hist(srankN, bins=30, range=(0, 1),
                                 density=True, color=color)
    for item in p:
        item.set_height(item.get_height()/sum(x))
    ...
    axes[0][2].set_ylim([-0.02*0.85, 0.85])

    nrankN = effectiveRanksDF['NuclearRank'] / effectiveRanksDF['Size']
    x, bins, p = axes[1, 0].hist(nrankN, bins=30, range=(0, 1),
                                 density=True, color=color)
    for item in p:
        item.set_height(item.get_height() / sum(x))
    ...
    x, bins, p = axes[1, 1].hist(energyN, bins=30, range=(0, 1),
                                 density=True, color=color)
    for item in p:
        item.set_height(item.get_height() / sum(x))
    ...
    x, bins, p = axes[1, 2].hist(elbowN, bins=30, range=(0, 1),
                                 density=True, color=color)
    for item in p:
        item.set_height(item.get_height() / sum(x))
    ...
    x, bins, p = axes[2, 1].hist(thrankN, bins=30, range=(0, 1),
                                 density=True, color=color)
    for item in p:
        item.set_height(item.get_height() / sum(x))
    ...
    x, bins, p = axes[2, 2].hist(shrankN, bins=30, range=(0, 1),
                                 density=True, color=color)
    for item in p:
        item.set_height(item.get_height() / sum(x))
    ...
    axes[2][2].set_ylim([-0.02*0.5, 0.5])
    ...
    axes[0, 0].xaxis.set_label_coords(1.05, -0.025)
    axes[0, 1].set_xlabel('rank/N')
    # axes[0, 1].xaxis.set_label_coords(0.5, -0.1)
    axes[0, 2].set_xlabel('srank/N')
    # axes[0, 2].xaxis.set_label_coords(0.5, -0.1)
    axes[1, 0].set_xlabel('nrank/N')
    # axes[1, 0].xaxis.set_label_coords(0.5, -0.1)
    axes[1, 1].set_xlabel('energy/N')
    # axes[1, 1].xaxis.set_label_coords(0.5, -0.1)
    axes[1, 2].set_xlabel('elbow/N')
    # axes[1, 2].xaxis.set_label_coords(0.5, -0.1)
    axes[2, 0].set_xlabel('erank/N')
    # axes[2, 0].xaxis.set_label_coords(0.5, -0.1)
    axes[2, 1].set_xlabel('thrank/N')
    # axes[2, 1].xaxis.set_label_coords(0.5, -0.1)
    axes[2, 2].set_xlabel('shrank/N')         
    # axes[2, 2].xaxis.set_label_coords(0.5, -0.1)
